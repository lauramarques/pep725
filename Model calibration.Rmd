...
```{r, include=FALSE}
# Minimum temperature
tmin.df <- data.table::fread("~/data/pep/calib/MinimumTemperature.csv") # Data from Michael
...
photo.df <- data.table::fread("~/data/pep/calib/Photoperiod_daylength.csv") # Data from daylength
...
save(DataList, file = "~/data/pep/datalist.RData")
save(phenor_input, file = "~/data/pep/phenor_input.RData")
save(df_tseries_id, file = "~/data/pep/df_tseries_id.RData")
save(preds.df, file = "~/data/pep/preds.df.RData")
...
#args <- as.character(df_tseries_id$idx[which(df_tseries_id$timeseries == "1_Quercus_robur")]) # args <- c("2261") 
...
ts <- paste0(site,"_",sp) 
data.sub <- DataList[[sp]][[as.character(site)]] # myDataList_perSta[[sp]][[as.character(site)]] #
...
save(DoYoff_Preds.df, file = "~/data/pep/DoYoff_Preds.df_1_Fagus_sylvatica.RData")
save(opt_pars.df, file = "~/data/pep/opt_pars.df_1_Fagus_sylvatica.RData")
```
...
pred_DoYoff <- data.table::fread("~/zanid90-AutumnPhenology/AutumnPhenology_Models&Analyses/Data/ModelAnalysis_1_Predicted_DoYoff_subset.csv") 
subsetPred_1 <- subset(pred_DoYoff, timeseries=="1_Fagus sylvatica")

# Loading calibration outputs for one timeseries
load("~/data/pep/DoYoff_Preds.df_1_Fagus_sylvatica.RData")
load("~/data/pep/opt_pars.df_1_Fagus_sylvatica.RData")

DoYoff_Preds.df_site_species <- DoYoff_Preds.df
opt_pars.df_site_species <- opt_pars.df
...
```{r,echo=FALSE, fig.height=6,fig.width=8, eval=FALSE}
...
save(DoYoff_Preds.df, file = "~/data/pep/DoYoff_Preds.df_1_Quercus_robur.RData")
save(opt_pars.df, file = "~/data/pep/opt_pars.df_1_Quercus_robur.RData")
```
...
# Loading calibration outputs for one timeseries
load("~/data/pep/DoYoff_Preds.df_1_Quercus_robur.RData")
load("~/data/pep/opt_pars.df_1_Quercus_robur.RData")
